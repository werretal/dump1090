...
PlaneObject.prototype.updateMarker = function(moved) {
	...
	if (this.marker) {
			...
			this.ibox.setPosition(this.position);
                this.updateIcon();
	} else {
                this.updateIcon();
		this.marker = new google.maps.Marker({
			...
			visible: true
                ...
                if (this.flight !== null) {
                  var icallsign = this.flight;
                } else {
                  var icallsign = "unknown";
                }    
                   
                this.ibox = new InfoBox({
                    content: icallsign,
                    position: this.position,
                    boxStyle: {
                    ...
                    ,disableAutoPan: true
                    ,pixelOffset: new google.maps.Size(11, 5)
                    ,closeBoxURL: ""
                    ,isHidden: false
                    ,pane: "mapPane"
	...
	// Setting the marker title '<div style="font-size:8px>"'+(this.flight)+'</div>';
        var title = (this.flight === null || this.flight.length == 0) ? this.icao : (this.flight+' ('+this.icao+')');
        if (title !== this.marker.title)
	        this.marker.setTitle(title);
	        if (this.flight !== null) {
                  var wcallsign = this.flight;
                } else {
                  var wcallsign = "unknown";
                }    
	        this.ibox.setContent(wcallsign);        
};
